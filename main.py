...
class myGui:
    def __init__(self):
        # Lock file to prevent multiple instances of key2click opening
        self.lockfile = Path(os.getenv("TEMP")) / "Key2Click.lock" 
        self.button_fg = "#333333"
        if self.lockfile.exists(): #Ideally, if there is already another instance open, show an error
            messagebox.showerror(
                "Already Running",
                "Key2Click is already open!\n\nPlease check your taskbar or system tray.",
            )
            sys.exit(0)
        # Create lock file since it doesn't exist
        try:
            self.lockfile.touch() # touch :) 
        except Exception:
            pass
        #Failsafe is a way to prevent your mouse from losing control during automation
        #Whenever anything crazy happens, just move your mouse to any of the corners of your screen, and the script stops
        pyautogui.FAILSAFE = False #It's best to keep this off for our needs
        # Where to save shortcuts
        self.appdata = os.getenv("APPDATA")
        self.config_dir = Path(self.appdata) / "Key2Click"
        self.config_dir.mkdir(exist_ok=True)
        self.config_file = self.config_dir / "shortcuts.json" #shortcuts stored here

        self.is_running = False
        self.default_position = None
        self.shortcuts_dictionary = {}

        self.root = tk.Tk() #App creation 
        ...
        #app width and height is calculated based on the screen size, new update on v1.0.1
        #hardcoding the app size in pixels can cause different app sizes on different PCs
        ...
        self.root.geometry( #Attempt to make the app launch at the center of the screen
            f"{self.app_width}x{self.app_height}+{self.center_x}+{self.center_y}"
        )
        ...
        )  # When closing window - call self.on_close() function

        self.main = tk.LabelFrame(
            master=self.root, border=0, bg="#f5f5f5"
        )  # Everything stays inside here
        self.header = tk.LabelFrame(master=self.main, border=0, bg="#f5f5f5")
        self.main_label = tk.Label(
            master=self.header, text="Key2Click", font="Calibri 20 bold", bg="#f5f5f5"
        )

        self.help_icon_pack = self.resource_path("help icon.gif")
        self.help_icon = tk.PhotoImage(file=self.help_icon_pack)
        self.icon = tk.Label(
            master=self.header, image=self.help_icon, fg="blue", cursor="hand2"
        )
        self.icon.bind(
            "<Button-1>", self.show_help
        )  # When icon is clicked, call show_help()
        #Position Frames on the app
        ...
        self.create_shortcut = tk.Button(
            master=self.main,
            text="Select point",
            command=self.add_map_point,
            cursor="hand2",
            fg=self.button_fg,
            bg="white",
        )  # Select point button
        self.selected_point = tk.Label(
            master=self.main, text="No point selected", bg="#f5f5f5"
        )  # Displays the point (x,y co-ordinates) that was selected
        ...
        self.shortcut_entry_frame = tk.LabelFrame(
            master=self.main, borderwidth=0, border=0
        )
        self.enter_shortcut = tk.Label(
            master=self.shortcut_entry_frame, text="Enter shortcut", bg="#f5f5f5"
        )  # Label describing it
        self.enter_shortcut.grid(row=0, column=0, padx=(0, 5))
        self.shortcut_entry = tk.Entry(
            master=self.shortcut_entry_frame, bd=1
        )  # Entry for it
        ...
        self.add_shortcut = tk.Button(
            master=self.main,
            text="Add",
            command=self.add_shortcut_to_list,
            cursor="hand2",
            fg=self.button_fg,
            bg="white",
        ) #The add shortcut button - adds valid shortcuts to the "loaded shortcut" section
        self.add_shortcut.grid(row=4, column=1, pady=(0, 20))

        # Section that contains working shortcuts
        self.shortcuts = tk.LabelFrame(
            master=self.main,
            border=0,
        ...
        self.working_shortcuts = tk.Label(
            master=self.shortcuts,
            text="Loaded shortcuts",
            font="Calibri 16",
            border=0,
            bg="#f5f5f5",
        )
        ...
        self.shortcut_list = tk.LabelFrame(
            master=self.shortcuts, border=0, highlightthickness=0, relief="flat"
        )
        ...
        #4 option buttons down below open_saved, export_to, delete_shortcut, edit_shortcut
        self.open_saved = tk.Button(
            master=self.options,
            text="Open saved",
        ...
        self.delete_shortcut = tk.Button(
            master=self.options,
        ...
        self.edit_shortcut = tk.Button(
            master=self.options,
            text="Edit Point",
            command=self.edit_selected_point,
            cursor="hand2",
            fg=self.button_fg,
            bg="white",
        )
        ...
        footer_label = tk.Label(
            master=self.footer, image=self.linkedin_img_p, bg="#f0f0f0", cursor="hand2"
        )
        footer_label.bind(
            "<Button-1>",
            lambda e: self.open_url("https://www.linkedin.com/in/emmanuel-pogbe/"),
        ) #IDK why I put my linkedin handle in the app, probably not very smart
        footer_label.pack(side=tk.RIGHT, padx=5, pady=5)

        # Bring back auto_saved shortcuts when app launches
        self.auto_saved_shortcuts()

        self.main.pack()
        self.footer.pack(side=tk.BOTTOM, fill=tk.X)
        self.root.mainloop() # Hats down for this line of code for powering this whole app

    def on_close(self):
        """Clean up lock file when closing""" 
        try: 
            #This can break sometimes (unexpected shutdowns), causing the lockfile to remain even though app is closed
        ...
        if not self.is_running:
            if len(self.shortcuts_dictionary) > 0:
                ...
                self.start_btn.config(
                    text="STOP", activebackground="#A50000", bg="#D10000"
                )
                ...
                self.hotkey_map = {  # I love copilot so much
                    short: (lambda s=short: self.click_point(s))
                    for short in self.shortcuts_dictionary
                }
    ...
    def configure_selection_window(self, is_selecting):
        if is_selecting:
            ...
            self.root.geometry(
                f"{self.app_width}x{self.app_height}+{center_x_1}+{center_y_1}"
            )

    def add_map_point(self):  # Will probably refactor this later
        proceed = messagebox.askokcancel(
            "Note",
            "Click at a point where you want to set a shortcut\n(after clicking 'ok' of course)",
        )
        if proceed:
            ...
            self.selected_point.config(
                text=f"Coordinates -> {coordinate_x},{coordinate_y}"
            )
            # self.selected_point.grid(row=1,column=1)
    ...
    def add_shortcut_to_list(self):
        if not self.default_position:
            ...
            if not self.is_available(shortcut=shortcut):
                messagebox.showinfo(
                    "Note",
                    "Shortcut is already mapped to a point! Enter another one\nNote: Shortcuts are case-insensitive",
                )
            elif not self.is_valid_shortcut(shortcut):
                messagebox.showinfo(
                    "Note",
                    "Please enter a valid shortcut! Here are some ideas (the '+' is very important)\nctrl+f\nalt+p\nshift+1\nctrl+alt+3\nz\n9\nShortcuts are case-insensitive",
                )
            else:
    ...
    def is_available(self, shortcut):
        ...
        all_shorts = (
            shortcut_info.split()[0] for shortcut_info in self.get_all_shortcuts()
        )
    ...
    def insert_listbox(self, shortcut, position):
        self.shortcuts_listbox.insert(
            0, self.format_mapping(shortcut=shortcut, position=position)
        )
    ...
    def delete_selected(self):
        ...
        if len(selected) >= 1:
            for item in reversed(
                selected
            ):  # reverse the list so that we can delete from the end so as not to affec the current list
            ...
            messagebox.showinfo(
                "No shortcut selected", "You have not selected any shortcut to delete!"
            )
            ...
            try:
                with open(self.filename, "r") as shortcuts:
                    ...
                    for shortcut in self.shorts:
                        ...
                        if (
                            self.is_valid_shortcut(shortcut)
                            and isinstance(points, list)
                            and len(points) == 2
                            and all(isinstance(x, int) for x in points)
                            and points[0] <= screen_size[0]
                            and points[1] <= screen_size[1]
                        ):
                    ...
                    if is_corrupt:
                        messagebox.showwarning(
                            ...
                            "Some shortcuts were not loaded! "
                        ...
                        messagebox.showinfo(
                            "Unloaded shortcuts",
                            f"{duplicates_} already present in loaded shortcuts",
                        )
            except Exception as e:
                messagebox.showerror(
                    "Read error",
                    f"Oops! Something went wrong {e.__class__.__name__} {e}",
                )

    def export_to_json(self):
        if len(self.shortcuts_dictionary) < 1:
            messagebox.showinfo(
                "No loaded shortcuts", "There are no loaded shortcuts to export"
            )
        else:
            try:
                json_file = filedialog.asksaveasfile(
                    mode="w",
                    defaultextension="json",
                    filetypes=[("JSON files", "*.json")],
                    title="Export loaded shortcuts",
                )
                if json_file:
                ...
                messagebox.showerror(
                    "Error", f"Oops! Something went wrong: {e.__class__.__name__}"
                )
    ...
    def get_shortcut_dictionary(self):
        ...
        #Convert the shortcut strings gotten from listbox into a shortcut dictionary 
        short_dict = {
            short: self.listify(mapping)
            for short, mapping in (shortcut.split() for shortcut in all_shortcuts)
        } #This was my first time doing a comprehension this complicated
        return short_dict

    def edit_selected_point(self):
        selected = self.shortcuts_listbox.curselection()
        #If you've selected a shortcut to edit...
        if len(selected) == 1:
            ...
            self.shortcuts_listbox.insert(
                selected[0], self.format_mapping(start_str, self.default_position)
            )
        #If there are multiple shortcut selections
        elif len(selected) > 1:
            messagebox.showinfo(
                "Multiple selections", "You can only edit one shortcut point at a time"
            )
        #If there are no shortcut selections 
        else:
            messagebox.showinfo(
                "No shortcut selected", "You have not selected any shortcut to edit!"
            )
    ...
    def click_point(self, shortcut_key):
        ...
        try:
            #Move to the co-ordinates and simply click
            pyautogui.moveTo(coordinate_x, coordinate_y)
            pyautogui.leftClick()
        except pyautogui.FailSafeException:
            #This except block is probably useless since I switched it off earlier
            #But I'm not going to touch it since it's not disturbing anybody
            # `if it works, don't touch it`
            pass
        except Exception as e:
            messagebox.showerror(
                "Error", f"Something went wrong: {e.__class__.__name__} {e}"
            )

    def is_valid_shortcut(self, s: str):
        try:
            HotKey.parse(s) #Using the parse method to easily check if a shortcut is valid
            #If a shortcut is not valid, then it will raise a ValueError
            return True
        except Exception:
            # We can have more try except blocks in this place to make input handling more robust
            return False

    def listify(self, s: str) -> list:
        # Takes string having a tuple-like structure and turns it into a list compatible with json
